...
class NewReference extends React.Component {
  ...
  handleSave = async () => {
    this.validateInput();
    if (!this.state.titleError && !this.state.arTitleError) {
      const result = await AdminServices.referencesServices.createNewReference(
        this.state.title
      );
      if (result) {
        this.setState({
...
export default withMyHook(NewReference);
